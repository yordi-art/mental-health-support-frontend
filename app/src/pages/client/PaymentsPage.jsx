...
const statusCfg = {
  ...
  failed:   { icon: XCircle,     badge: 'bg-red-100 text-red-600' },
  refunded: { icon: RefreshCw,   badge: 'bg-purple-100 text-purple-700' },
  awaiting: { icon: Clock,       badge: 'bg-blue-100 text-blue-700' },
};
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6">
          ...
          <div className="space-y-3 text-sm mb-5">
            {[
              ['Receipt No.', p.ref || p._id],
              ['Therapist', p.therapistName || p.therapist],
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="divide-y divide-gray-50">
            {filtered.map(p => {
              ...
              const mColor = methodColor[p.method] || 'text-gray-500 bg-gray-50';
              return (
                <div key={p._id || p.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 transition">
                  <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
                    {(p.therapistName || p.therapist || 'T')[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-700 truncate">{p.therapistName || p.therapist}</p>
